...
import { checkInAppointment, createWalkInPatient, fetchTodayAppointments, prioritizeToken, rescheduleAppointment, } from './services/receptionistApi';
export const ReceptionistDashboardPage = () => {
    ...
    const handleCheckIn = async (appointmentId) => {
        try {
            ...
            if (result.success) {
                ...
                setAppointments(prev => prev.map(apt => (apt.id === appointmentId ? { ...apt, status: 'Arrived' } : apt)));
            }
        }
        catch (err) {
    ...
    const handleReschedule = async (appointmentId) => {
        ...
        try {
            ...
            if (result.success) {
                ...
                setAppointments(prev => prev.map(apt => (apt.id === appointmentId ? { ...apt, timeSlot: newSlot } : apt)));
            }
        }
        catch (err) {
    ...
    const handleEmergencyPriority = async (tokenId) => {
        try {
    ...
    return (<div style={{ padding: '24px', fontFamily: 'sans-serif', maxWidth: '950px', margin: '0 auto' }}>
      ...
      <section style={{ background: '#f8f9fa', padding: '16px', borderRadius: '8px', marginBottom: '24px', border: '1px solid #e9ecef' }}>
        ...
        <form onSubmit={handleWalkInSubmit} style={{ display: 'flex', gap: '12px' }}>
          <input type="text" placeholder="Patient Name" value={patientName} onChange={e => setPatientName(e.target.value)} style={{ padding: '8px', flex: 1 }} required/>
          <input type="text" placeholder="Phone Number" value={phone} onChange={e => setPhone(e.target.value)} style={{ padding: '8px', flex: 1 }} required/>
          <button type="submit" style={{ padding: '8px 16px', backgroundColor: '#0d6efd', color: '#fff', border: 'none', borderRadius: '4px', cursor: 'pointer' }}>
            Generate Token
          </button>
        </form>
        {generatedToken && (<div style={{ marginTop: '12px', padding: '12px', background: '#e7f5ff', border: '1px solid #a5d8ff', borderRadius: '4px' }}>
      ...
      <section style={{ background: '#fff', padding: '16px', borderRadius: '8px', border: '1px solid #dee2e6' }}>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {appointments.map(apt => (<tr key={apt.id} style={{ borderBottom: '1px solid #e9ecef' }}>
                ...
                <td style={{ padding: '8px', display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                  {apt.status !== 'Arrived' && (<button onClick={() => handleCheckIn(apt.id)} style={{ padding: '4px 10px', backgroundColor: '#198754', color: '#fff', border: 'none', borderRadius: '4px', cursor: 'pointer' }}>
                      Mark Arrived
                    </button>)}
                  {/* F-3.3 Reschedule Action */}
                  <button onClick={() => handleReschedule(apt.id)} style={{ padding: '4px 10px', backgroundColor: '#0dcaf0', color: '#000', border: 'none', borderRadius: '4px', cursor: 'pointer' }}>
                    📅 Reschedule
                  </button>
                  {/* F-3.4 Emergency Priority Action */}
                  <button onClick={() => handleEmergencyPriority(apt.id)} style={{ padding: '4px 10px', backgroundColor: '#dc3545', color: '#fff', border: 'none', borderRadius: '4px', cursor: 'pointer' }}>
                    🚨 Priority
                  </button>
                </td>
              </tr>))}
