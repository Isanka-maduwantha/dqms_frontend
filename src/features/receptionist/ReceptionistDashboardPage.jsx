...
export const ReceptionistDashboardPage = () => {
  ...
  const handleReschedule = async (appointmentId) => {
    const newSlot = prompt(
      'Enter new time slot (e.g., "10:30 AM" or "02:00 PM"):',
    );
    if (!newSlot) return;
    try {
      ...
      if (result.success) {
        ...
        setAppointments((prev) =>
          prev.map((apt) =>
            apt.id === appointmentId ? { ...apt, timeSlot: newSlot } : apt,
          ),
        );
      }
    } catch (err) {
    ...
    >
      ...
      >
        ...
        {generatedToken && (
          ...
          >
            ...
            <span style={{ fontSize: "1.2rem", color: "#1864ab" }}>
              {generatedToken}
            </span>
          </div>
        )}
      ...
      >
        ...
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead>
            <tr
              style={{ borderBottom: "2px solid #dee2e6", textAlign: "left" }}
            >
                ...
                <td style={{ padding: "8px" }}>
                  <span
                    style={{
                      ...
                      fontSize: "12px",
                      backgroundColor:
                        apt.status === "Arrived" ? "#d1e7dd" : "#fff3cd",
